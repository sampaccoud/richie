...
class CreateCmsDataTests(CMSTestCase):
    ...
    def test_command_success(self, mock_create, mock_clear, mock_logger):
        ...
        Note:
            This test can pass with success even if the create_demo_site site
            fails on "real" usage from commandline like with a bug on a
            factory this test pass although commandline usage fails.
        """
    ...
    def test_command_create_demo_site(
        self, mock_page, mock_course, mock_organization, mock_person,
        mock_subject
    ):
        """
